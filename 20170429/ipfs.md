...
我曾经希望完全使用Git的对象格式，但这必定无法再引入某些分布式文件系统里有用的特性了，即(a)快速大小查找(字节大小已被聚合添加到对象里)，(b)大文件去重(添加一个列表对象)，(c)把提交也嵌入到树上。不过，IPFS文件的对象和Git的对象已经像到足够互相转换了。而且将Git对象被引入到IPFS中，转换后不会丢失任何信息(比如unix文件权限等)。
