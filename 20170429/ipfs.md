...
<center>Juan Benet</center>
<center>juan@benet.ai</center>
...
我曾经希望完全使用Git的对象格式，但这必定无法再引入某些分布式文件系统里有用的特性了，即(a)快速大小查找(聚合字节的大小已被添加到对象里)，(b)大文件去重(添加一个列表对象)，(c)把提交也嵌入的树上。然而，IPFS文件的对象和Git的对象已经像到足够将它们互相转换了。此外，Git对象能被引入再转换后而不丢失任何信息(比如unix文件权限等)。
