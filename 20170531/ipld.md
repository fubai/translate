...
- **_merkle-links_**: 默克尔图结构的核心单位。
- **_merkle-dag_**: 边缘是_merkle-link_的图。
- **_merkle-paths_**: unix风格的路径，以_命名化的merkle-link_在_merkle-dag_之间穿行。
- **IPLD数据模型**: 一个灵活的，基于JSON的数据模型，用于表示merkle-dag。
- **IPLD序列化格式**: 可以表示IPLD对象的一些列格式，比如JSON，CBOR，CSON，YAML，Protobuf，XML，RDF等等。
- **IPLD标准格式**: 序列化格式的确定性描述，用于确保相同的_logical_对象始终被序列化为_完全相同的bit序列_。这对默克尔式的连接以及所有的加密应用来说至关重要。
...
_merkle-link_是两个对象之间的链接，它们以目标对象的内容算出_密码式的哈希_，并以其做寻址，这些链接会嵌入到源对象里。用merkle-link以内容做寻址能：
...
IPLD对象模型里用map表示一个_merkle-link_，这个map只有一个键`/`，它被映射到一个"链接值"。比如：
...
具有merkle-link的对象会形成图(merkle-graph)，这个对象和这个图都必须是定向的，能不被计算成环，如果密码哈希函数的属性成立的话。这就是_merkle-dag_。因此所有用了_merkle来做连接_的图(_merkle-graph_)必须是一个有向无环图(DAG是有向无环图的简称，所以才有了_merkle-dag_)。

### 什么是_merkle-path_？

merkle-path是一个unix风格的路径(比如`/a/b/c/d`)，它最初通过_merkle-link_解析引用，可以传递性的访问被引用的节点和其它节点。

通用的文件系统可以在IPLD的上层设计一种对象模型，能专用于文件维护并有专有的路径算法可以在这个模型上做查询。

### _merkle-paths_如何工作？

_merkle-path_是一种unix风格的路径，它的找寻过程是先通过_merkle-link_解析引用，然后跟随被引用的中间对象中的_被命名的merkle-link_。跟随一个名字意味着要到对象里面去查找，先找到这个_name_，然后解析成关联的_merkle-link_。
...
IPLD数据模型为所有的merkle-dag定义了一个简单的基于JSON的_结构_，并且标识了一组可用于编码这个结构的格式。
...
IPLD允许用户构建复杂的数据结构，用其他和链接关联的属性。这能很有用的把其他信息和链接编码在一起，比如关系信息，或者链接需要的辅助信息。这和“链接对象约定”是_不同_的，下面会讨论“链接对象约定”，在它们正确的情况是很有用的。有时候只需要在链接上添加一点数据，不必添加另一个对象。通过简单的在一个对象上嵌入真实的IPLD链接就可以做到，这个链接上会有额外的属性。
...
尽管在_具体到这种数据结构_时，链接上有了新的属性，但还是能很好的解析链接：
...
但是在获取链接时不能像获取其它属性一样舒服了，因为链接意味着_(resolve through)_。

#### 重复的属性键

请注意，具有_相同_名字的两个属性是不允许的，但实际上无法阻止(有些人就会这样做，然后提供给解析器)，所以为了安全起见，我们把用于遍历路径的值定义为，序列化内容里的_第一个_入口。比如，假设有如下对象：
...
假设_这_就是_标准格式_(不是json，是cbor)里的_正确顺序_，它的哈希值是`QmDDD...DDD`，我们_总会_得到：
...
Unix和Web的路径描述出现了一些重要的问题。请看[这个讨论](https://github.com/ipfs/go-ipfs/issues/1710)。为了兼容unix和web的模式和期望，IPLD明确禁止带有了某些路径零件的路径。**请注意数据本身_也许_还带有这些属性(有人会这么做，有合理的用处)。所以只是_路径解析器_禁止解析这样的路径。**这样的限制在典型的unix和UTF-8路径系统里也一样：
...
为了利用JSON的成功，IPLD_直接兼容_了JSON，但是IPLD需要不被JSON的错误_阻碍_。这是我们能够为遵循格式的惯用选择而付出的，但是必须注意要确保总有一个定义良好的1:1的映射。
